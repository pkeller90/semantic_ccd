...
class VecPairDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = 0.0 if dat["type"] <= 0 else 1.0
        return vec1, vec2, float(label)
...
def learn(data_path, image_path, pairs_path, epochs=25):
...
if __name__ == "__main__":
    ...
    pairs_path = "<path>/datasets/%s/pairs.csv" % (dataset) # Path must be set
