...
def generate_non_clones(code_set, limit=None):
    ...
    func_sets = [code_set[code_set["func"] == x].sample(frac=1) for x in functionalities]
    non_clones_list = []
...
def extract_clone_pairs(functionalities, type4_only=True, limits=None, artifical_non_clones=True):
...
def extract_generalization_set(funcs_train, funcs_test, t4_only, limits, artifical_non_clones):
    train_pairs, train_code = extract_clone_pairs(funcs_train, t4_only, limits, artifical_non_clones)
    test_pairs, test_code = extract_clone_pairs(funcs_test, t4_only, limits, artifical_non_clones)
...
if __name__ == "__main__":
    # Selection of functionalities that may be most suitable for my approach
    #
    # 7 - bubble-sort array
    # 13 - shuffle array inplace
    # 44 - palindrome
    functionalities = [7, 13, 44]
    pairs, code = extract_clone_pairs(functionalities, False, {"0":100, "1": 000, "2": 0, "3": 0, "4": 100})
    ...
    pairs, code = extract_generalization_set(train_functionalities, test_functionalities, False, {"0":500, "1": 0, "2": 0, "3": 0, "4": 500})
    pairs.to_csv(folder_path + "/pairs.csv")
    code.to_csv(folder_path + "/funcs.csv")
